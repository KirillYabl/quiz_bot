...
def manage_menu_logic(bot, update, user_data):
    ...
    if update.message.text == 'Новый вопрос':
        logger.debug('User press new question')
        return Buttons.QUESTION

    if update.message.text == 'Сдаться':
        logger.debug('User press gave up')
        return Buttons.MENU
...
def check_answer(bot, update, user_data):
    ...
    if update.message.text == 'Сдаться':
        ...
        bot.send_message(chat_id=update.message.chat_id, text=msg)
        logger.debug('"Gave up" message sent')
        return Buttons.QUESTION

    msg = 'К сожалению нет! Правильный ответ:\n{}\nХотите новый вопрос? Выберите в меню.'.format(user_data['answer'])
    bot.send_message(chat_id=update.message.chat_id, text=msg)
    logger.debug('"Mistake" message sent')
    return Buttons.QUESTION
...
if __name__ == '__main__':
    ...
    logger.debug('Connection with TG established')

    # add handlers
    updater.dispatcher.add_handler(conv_handler)
    logger.debug('Conversation handler added to updater')
